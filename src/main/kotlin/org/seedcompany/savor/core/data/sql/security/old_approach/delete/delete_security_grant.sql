-- trigger function that runs when record in sys_role_grants table is deleted.
...
begin
	...
	raise info 'table: % - column: %', security_table_name, security_column_name;
		...
		for rec1 in (select sys_person_id from sys_role_memberships
			...
			select get_new_access_level(rec1.sys_person_id, old.table_name, old.column_name) into new_access_level;

			-- TODO: check if the function works without the if check.
