...
begin
	...
	order by table_name) loop 
        ...
        execute format('create table if not exists '|| security_table_name || ' ( __person_id int not null, __id int not null, __is_cleared boolean, foreign key(__person_id) references public.people_data(id), foreign key (__id) references ' ||  p_table_name || '(id))' );
